...
# Add project root and Kronos submodule to path
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
sys.path.insert(0, _project_root)
sys.path.insert(0, os.path.join(_project_root, "Kronos"))
...
class PredictionService:
    ...
    def _calculate_days_to_fetch(self, timeframe_minutes, target_bars=None):
        ...
        # Also enforce minimum of 4 days to handle Monday mornings (Sat/Sun have no data)
        min_days = 4
        days_to_fetch = max(min(days_with_buffer, 60), min_days)
