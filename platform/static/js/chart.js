...
const chartOptions = {
    width: 800,
    height: Math.max(window.innerHeight - 140 - VOLUME_CHART_HEIGHT, 350),
    layout: {
...
// Send chat message
/**
 * Capture the chart area as a base64 PNG screenshot
 * Uses html2canvas to render the charts-wrapper div
 * Returns base64 string (without data:image/png;base64, prefix) or null on failure
 */
async function captureChartScreenshot() {
    try {
        const chartsWrapper = document.querySelector('.charts-wrapper');
        if (!chartsWrapper) {
            console.warn('Charts wrapper not found for screenshot');
            return null;
        }

        const canvas = await html2canvas(chartsWrapper, {
            backgroundColor: '#1e222d',
            scale: 1,  // 1x scale to keep payload reasonable
            logging: false,
            useCORS: true,
            allowTaint: true,
        });
...
async function sendChatMessage() {
    ...
    try {
        ...
        // Capture chart screenshot in parallel with building the request
        const chartScreenshot = await captureChartScreenshot();
        ...
        if (chartScreenshot) {
            requestBody.chart_screenshot = chartScreenshot;
        }
