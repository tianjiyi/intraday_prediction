...
def setup_ddp():
    """
    Initializes the distributed data parallel environment, or falls back to
    single-GPU mode if DDP is not available.
    ...
    Returns:
        tuple: A tuple containing (rank, world_size, local_rank, use_ddp).
               use_ddp is False if running in standalone mode.
    """
    # Check if we're running under torchrun (DDP environment variables present)
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ and "LOCAL_RANK" in os.environ:
        if not dist.is_available():
            raise RuntimeError("torch.distributed is not available but DDP env vars are set.")
        ...
        print("[Standalone Setup] Running in single-GPU mode (no DDP)")
        rank = 0
        world_size = 1
        local_rank = 0
        if torch.cuda.is_available():
            torch.cuda.set_device(0)
            print(f"[Standalone Setup] Using GPU 0: {torch.cuda.get_device_name(0)}")
        else:
            print("[Standalone Setup] WARNING: No GPU available, using CPU")
...
def reduce_tensor(tensor: torch.Tensor, world_size: int, op=dist.ReduceOp.SUM) -> torch.Tensor:
    ...
    In standalone mode, returns the tensor unchanged.
    ...
    """
    # If not in distributed mode, return tensor as-is
