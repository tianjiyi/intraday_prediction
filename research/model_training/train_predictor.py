"""
Train Kronos Predictor on QQQ Intraday Data
Adapted from Kronos/finetune/train_predictor.py
"""
import os
import sys
import json
...
def create_dataloaders(config: dict, rank: int, world_size: int, use_ddp: bool):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=config['batch_size'], sampler=train_sampler, shuffle=shuffle_train,
...
def train_model(model, tokenizer, device, config, save_dir, rank, world_size, use_ddp, start_epoch=0, start_step=0, resume_state=None):
    """
    The main training and validation loop for the predictor.

    Args:
        start_epoch: Epoch to start from (0 for fresh training, >0 for resume)
        start_step: Step within epoch to start from (0 for epoch start, >0 for mid-epoch resume)
    ...
    for epoch_idx in range(start_epoch, config['epochs']):
        ...
        for i, (batch_x, batch_x_stamp) in enumerate(train_loader):
            ...
            # Access model.module only in DDP mode
            model_for_loss = model.module if use_ddp else model
            loss, s1_loss, s2_loss = model_for_loss.head.compute_loss(logits[0], logits[1], token_out[0], token_out[1])
            ...
            # Mid-epoch checkpointing (Master Process Only)
            checkpoint_interval = config.get('checkpoint_every_n_steps')
            if rank == 0 and checkpoint_interval is not None and (batch_idx_global + 1) % checkpoint_interval == 0:
                checkpoint_path = f"{save_dir}/checkpoints/checkpoint_epoch_{epoch_idx + 1}_step_{i + 1}.pt"
                ...
                print(f"[OK] Mid-epoch checkpoint saved: {checkpoint_path}")
        ...
        with torch.no_grad():
            for batch_x, batch_x_stamp in val_loader:
                ...
                logits = model(token_in[0], token_in[1], batch_x_stamp[:, :-1, :])
                model_for_loss = model.module if use_ddp else model
                val_loss, _, _ = model_for_loss.head.compute_loss(logits[0], logits[1], token_out[0], token_out[1])

                tot_val_loss_sum_rank += val_loss.item()
                val_batches_processed_rank += 1

        # Reduce validation metrics (only in DDP mode)
        if use_ddp:
...
def main(config: dict):
    ...
    if rank == 0:
        ...
        print("=" * 80)
        print("PREDICTOR TRAINING - QQQ INTRADAY")
        print("=" * 80)
    ...
    if config.get('resume_from_checkpoint') and os.path.exists(config['resume_from_checkpoint']):
        ...
        start_step = checkpoint.get('step', 0)  # Mid-epoch step (0 if end-of-epoch checkpoint)
        resume_state = {
        ...
        if start_step > 0:
            print(f"[Rank {rank}] Resuming from epoch {start_epoch + 1}, step {start_step} (best val loss: {resume_state['best_val_loss']:.4f})")
        else:
            print(f"[Rank {rank}] Resuming from epoch {start_epoch + 1} (best val loss: {resume_state['best_val_loss']:.4f})")
    else:
        # Fresh training
    ...
    if rank == 0:
        print(f"Predictor Model Size: {get_model_size(model_for_size)}")

    # Start Training
    dt_result = train_model(
        model, tokenizer, device, config, save_dir, rank, world_size, use_ddp,
        start_epoch=start_epoch, start_step=start_step, resume_state=resume_state
    )
...
if __name__ == '__main__':
    # Usage:
    # - Windows (standalone): python model_training/train_predictor.py
    # - Linux/Multi-GPU: torchrun --standalone --nproc_per_node=1 model_training/train_predictor.py
