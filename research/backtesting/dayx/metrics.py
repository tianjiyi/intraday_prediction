"""
Performance metrics — overall and per-strategy breakdown.
"""
...
def compute_metrics(trades: list[Trade], equity_curve: list[tuple],
    ...
    Args:
        ...
        strategy_filter: If set, only include trades from this strategy
...
def per_strategy_breakdown(trades: list[Trade], equity_curve: list[tuple],
                           initial_capital: float = 100_000.0) -> dict[str, dict]:
    """Compute metrics grouped by strategy name."""
    strategies = set(t.strategy for t in trades)
    result = {}
    for strat in sorted(strategies):
        result[strat] = compute_metrics(
            trades, equity_curve, initial_capital, strategy_filter=strat
        )
    return result
...
def print_summary(overall: dict, per_strat: dict[str, dict],
                  by_hour: dict, by_day: dict):
    """Pretty-print backtest results to console."""
    print("\n" + "=" * 70)
    print("DAY_X BACKTEST RESULTS")
    print("=" * 70)
