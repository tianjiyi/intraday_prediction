...
def generate_batch_samples_simple(
    ...
    # pred_df is expected to be a dict with 'samples' -> list of DataFrames
    batch_paths = []
    if isinstance(pred_df, dict) and 'samples' in pred_df:
        for sample_df in pred_df['samples']:
            arr = sample_df[['open', 'high', 'low', 'close', 'volume']].values
            batch_paths.append(arr)
    else:
        # Fallback to averaged path if samples not available
        path_data = pred_df[['open', 'high', 'low', 'close', 'volume']].values
        batch_paths = [path_data for _ in range(batch_size)]
